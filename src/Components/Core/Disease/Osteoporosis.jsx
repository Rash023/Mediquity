...
const OsteoporosisDetection = () => {
  ...
  const predictOsteoporosis = async () => {
    if (!selectedImage) {
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/predict/osteoporosis", {
        method: "POST",
        body: formData,
      });
      ...
      console.error("Error predicting Osteoporosis", error);
      alert("Error predicting Osteoporosis. Please try again later.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div>
          <div className="p-3 lg:p-0">
            ...
            <p className="select-none text-neutral-500 max-w-lg mx-auto text-lg lg:text-[1.25rem] tracking-[1px] font-ai p-3 lg:p-0 text-justify">
              ...
              , your personal Osteoporosis Detector. I specialize in providing support and guidance for individuals concerned about osteoporosis risk assessment. Whether you seek insights into symptoms, risk factors assessment, or preventive measures,{" "}
              <span className="uppercase font-bold floating-animation gemini-font">
                OsteoCheck
              </span>{" "}
              is here to assist you every step of the way. The name OsteoCheck reflects our dedication to osteoporosis awareness and proactive health management.
            </p>
          </div>
...
export default OsteoporosisDetection;
