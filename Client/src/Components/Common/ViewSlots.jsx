...
const ViewSlots = () => {
    ...
    const fetchSlots = async () => {
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/api/v1/doctor/getDoctorSlots?id=${doctorId}`
        );
  ...
  const handleConfirmBooking = async () => {
    try {
      await axios.post(
        `${BASE_URL}/api/v1/user/bookAppointment`,
        {
          doctorId: doctorId,
    ...
    <div className="min-h-screen">
      <div className="flex flex-col w-[100%]">
        <div className="flex xl:flex-row flex-col gap-y-10 mt-[8%] w-full lg:justify-center lg:gap-x-20 items-center">
          <div>
            <div className="animate-pulse bg-gray-300 rounded-full lg:h-[400px] h-[350px] lg:w-[400px] w-[350px]"></div>
          </div>
          ...
          <div className="flex items-center">
            <div className="flex flex-col gap-y-5 lg:justify-center items-center">
              <div className="flex lg:flex-row flex-col gap-x-5 lg:items-baseline items-center gap-y-3">
        ...
        <div className="flex flex-col w-[100%] xl:-mt-[27%]">
          <div className="mx-auto mt-[5%]">
            <h1 className="select-none text-gray-300 lg:text-5xl text-4xl uppercase first-letter:text-6xl tracking-[2px]">
              Available <span className="text-6xl tracking-[2px]">S</span>lot
            </h1>
          </div>
          ...
          <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-x-12 w-full lg:p-20 p-10 gap-y-4">
            {[...Array(3)].map((_, index) => (
              ...
              >
                <div className="animate-pulse bg-gray-300 h-8 w-2/4 rounded mx-auto"></div>
              ...
              <div className="flex flex-col gap-y-5 lg:justify-center items-center">
                <div className="flex lg:flex-row flex-col gap-x-5 lg:items-baseline gap-y-3">
                  ...
                  <div className="bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-xl lg:text-3xl font-bold uppercase select-none tracking-[1px] text-center">
                    Dr. {slotDetails?.data?.data[0].doctorId?.name}
                  </div>
                </div>
                ...
                <div className="flex lg:flex-row flex-col gap-x-5 lg:items-baseline gap-y-3">
                  ...
                  <div className="bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-xl lg:text-3xl font-bold uppercase select-none tracking-[1px] text-center">
                    {slotDetails?.data?.data[0].doctorId?.specialization}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-x-12 w-full lg:p-20 p-10 gap-y-4">
              {slotDetails?.data?.data?.map((slot, index) => (
                <div
                  key={index}
                  className={`${
                    slot.isFull ? "pointer-events-none" : "cursor-pointer"
                  } h-fit w-full flex flex-col gap-y-4 border border-white rounded-[15px] bg-black p-4`}
                  onClick={() => handleSlotClick(slot)}
                >
                  <div className="bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-xl lg:text-3xl font-bold uppercase select-none tracking-[1px] text-center lg:first-letter:text-4xl first-letter:text-2xl">
                    {slot.day}
                  </div>
                  <div className="bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-xl lg:text-2xl font-bold uppercase select-none tracking-[1px] text-center">
                    {slot.time}
                  </div>
                  {slot.isFull && (
                    <div className="flex lg:items-center gap-1 mx-auto">
                      <div className="text-red-500 text-md text-[1.2rem]">
                        <LuAsterisk />
                      </div>
                      <div className="text-neutral-500 my-1 text-[1.2rem] text-center tracking-[1.5px]">
                        N/A
                      </div>
                    </div>
                  )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2">
            ...
            <div className="relative bg-white p-8 rounded-md shadow-md">
              ...
              <p className="uppercase tracking-[1.2px]">
                Are you sure you want to book the slot for {selectedSlot.day} at{" "}
                {selectedSlot.time}?
              </p>
